...
fn verify_regpairs() {
    ...
    for (k, v) in GLOBAL_REGPAIR_MAP.iter() {
        let regpair_str = hexagon_global_reg_to_str(k);
        let reglo_str = hexagon_global_reg_to_str(&v.1);
        let reghi_str = hexagon_global_reg_to_str(&v.0);
...
fn test_regpair_helper(
    ...
    hex_regpair: impl Into<ArchRegister>,
    hex_hi_reg: impl Into<ArchRegister>,
    hex_lo_reg: impl Into<ArchRegister>,
) {
    ...
    let hex_regpair: ArchRegister = hex_regpair.into();
    cpu.write_register(hex_regpair.clone(), (HI << 32) | LO)
        .unwrap();
